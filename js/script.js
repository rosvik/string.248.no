...
function calculateAndPopulate(input) {
  const precieved_length = precievedLength(input);
  setElementContentById("length", precieved_length);

  const word_count = wordCount(input);
  setElementContentById("wordcount", word_count);

  const size = encoder.encode(input).length;
  setElementContentById("size", size);

  const md5Hash = hashMessage(input, "MD5");
  setElementContentByIdAsync("md5_out", md5Hash);

  const sha1Hash = hashMessage(input, "SHA-1");
  setElementContentByIdAsync("sha1_out", sha1Hash);

  const sha256Hash = hashMessage(input, "SHA-256");
  setElementContentByIdAsync("sha256_out", sha256Hash);

  const sha384Hash = hashMessage(input, "SHA-384");
  setElementContentByIdAsync("sha384_out", sha384Hash);

  const sha512Hash = hashMessage(input, "SHA-512");
  setElementContentByIdAsync("sha512_out", sha512Hash);

  const b64_encoding = b64Encode(input);
  setElementContentById("b64e_out", b64_encoding);
...
/**
 * Calculate the length of a string as it is percieved by a human, accounting
 * for astral symbols and combining marks.
 *
 ...
 * @returns {number} The precieved length of the string.
 */
function precievedLength(str) {
  ...
  const symbol_with_combining_marks = /(\P{Mark})(\p{Mark}+)/gu;

  // Remove any combining marks, leaving only the symbols they belong to
  let stripped = str.replace(
    symbol_with_combining_marks,
    (_, symbol, _combining_marks) => symbol
  );
  return Array.from(stripped).length;
}

/**
 * Calculate word count, where a word is defined as a sequence of word characters.
 *
 * Based on:
 * https://github.com/iarna/word-count
 *
 * @param {string} str The string to calculate the word count of.
